...
#+BEAMER_HEADER:   \node[anchor=north west,xshift = 8.4cm, yshift = -2.2cm] at (current page.north west) {\includegraphics[width=0.075\textwidth]{~/phd/Figs/talk/iaxo_logo.png}};
#+BEAMER_HEADER: \end{tikzpicture}
#+BEAMER_HEADER: \begin{tikzpicture}[remember picture, overlay]
#+BEAMER_HEADER:   \node[anchor=north west,xshift = 14cm, yshift = -2.4cm] at (current page.north west) {\includegraphics[height=.1\textheight]{~/phd/Figs/talk/CAST}};
#+BEAMER_HEADER: \end{tikzpicture}

#+BEAMER_HEADER: \begin{tikzpicture}[remember picture, overlay]
#+BEAMER_HEADER:   \node[anchor=south west,xshift = 1cm, yshift = 0.5cm] at (current page.south west) {\includegraphics[height=.15\textheight]{~/phd/Figs/talk/UNI_Bonn_Logo_Standard_RZ.pdf}};
#+BEAMER_HEADER: \end{tikzpicture}
#+BEAMER_HEADER: \begin{tikzpicture}[remember picture, overlay]
#+BEAMER_HEADER:   \node[anchor=south,xshift = 0cm, yshift = 0.5cm] at (current page.south) {\includegraphics[height=.15\textheight]{~/phd/Figs/talk/gasdet_logo.pdf}};
#+BEAMER_HEADER: \end{tikzpicture}
#+BEAMER_HEADER: \begin{tikzpicture}[remember picture, overlay]
#+BEAMER_HEADER:   \node[anchor=south east,xshift = -1cm, yshift = 0.5cm] at (current page.south east) {\includegraphics[height=.15\textheight]{~/phd/Figs/talk/FTD_Logo-001.jpg}};
#+BEAMER_HEADER: \end{tikzpicture}}
...
* Test talk #5 <2024-04-16 Tue>                                    :noexport:

This one was done with Cristina listening. It took 31:21.74 min.

It was pretty good all in all and Cristina really liked it!

* Test talk #6 <2024-04-18 Thu 13:04>                              :noexport:

Test talk done! This time it was 32:06.32 min!

I think this was one of the best talks so far! Which makes me hopeful
for the real talk, because all talks have been short enough and are
getting clearer and clearer!

* Test talk #7 <2024-04-19 Fri 11:43>                              :noexport:

Another talk finished! Time was 32:07.33 min.

I think this was also quite a clean presentation. I still vary a
little bit here and there in how I explain certain things, but I have
enough experience at this point to know how to deal with that.

* Test talk #8 <2024-04-19 Fri 18:00>                              :noexport:

Another test talk with Cristina. This one in the living room in front
of the TV. Took almost exactly 32 minutes again.

My only real complaint was having more issues looking at the
"audience".

* Test talk #9 <2024-04-21 Sun 21:57>                              :noexport:

First test talk done at home!
Took 33:30.40 min.

In principle it was good, but somehow I was quite a bit slower in the
beginning part (or really in the part up to the limit calculation).

Not quite sure why, but 33 min is still perfectly fine I guess.

- [ ] Need to fix a bunch of missing minuses for GeV^-1

* Test talk #10 <2024-04-22 Mon 23:58>                             :noexport:

Another test talk that took 33:21.35 min! At least not even longer.

I think I'm doing a good job, but because things are getting 'easier'
now, I'm becoming a bit slower / explain a few more things maybe?
Still reasonable, I would say.

* Test talk #11 <2024-04-24 Wed 16:56>                             :noexport:

Final test talk I just did at home. Took 31:50.67 min!

I think it was probably the best talk so far. I didn't have any real
hickups and I explained some things better than before. If I do it
like this tomorrow, I'll be happy. :)
...
\begin{tikzpicture}[remember picture, overlay]
\node[anchor=north west, xshift = 0.2cm, yshift = -0.75cm] at (current page.north west) {
  \begin{minipage}{5cm}
    \footnotesize\orange{Hint:} This means the network is both \emph{trained} and '\emph{applied}' based entirely on synthetic X-ray data!
  \end{minipage}
};
\end{tikzpicture}
# have to insert some negative space, because adding the above
# annotation for some reason breaks the layout of the slide a bit. 
\vspace{-0.3cm}
...
*** Axion-photon coupling
  \[
  g_{aγ, \text{observed}} \lesssim \SI{9.0e-11}{GeV^{-1}} \text{ at } \SI{95}{\%} \text{ CL}.
  \]
  (current best: $g_{aγ, \text{Nature, 2017}} < \SI{6.6e-11}{GeV^{-1}}$)
...
- MLP as classifier trained and applied based on synthetic X-ray data
