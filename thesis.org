...
[[#sec:preparation:cdl]]. The second are measurements using
$\ce{^{55}Fe}$ source installed on a pneumatic manipulator at CAST
...
#+CAPTION: The peak position in \si{V} of the photopeak in the $\ce{^{55}Fe}$ calibration
...
#+CAPTION: The mean rise time of the signals recorded during the $\ce{^{55}Fe}$ data
...
(*sec ...*) computes the number of electrons recorded on each GridPix
pixel in an event. In order to calculate an equivalent energy based on
a certain amount of charge, the data recorded using the $\ce{^{55}Fe}$
calibration source at CAST is used. As the $\ce{^{55}Fe}$ spectrum
...
and recorded charge is linear. This allows to deduce the linear
relationship between charge and energy seen in the escape and photo
peak. The position of the two peaks in the $\ce{^{55}Fe}$ spectrum
needs to be determined precisely, which is done using a double
gaussian fit
...
$G^{\text{esc}}$ and one for the photo peak.

- [X] *INSERT A 55 FE FIT W/ FIT PARAMETERS SHOWN*

#+CAPTION: Fit to a $\ce{^{55}Fe}$ calibration run from the CAST data (run 149)
...
Then, a linear function without y-offset is fitted for the calibration
factor. With this calibration factor all events in the data run can
then be calibrated.
...
ta the time of recording the $\ce{^{55}Fe}$ spectrum. Over the time of
...
understanding of the physical events, allowing calculation of it also
for background data. This motivates the idea to map a gas gain valid
for a certain time to a calibration factor needed to convert events at
such gas gains to energies. This is achieved by first performing the
aforementioned gaussian fits to _all_ calibration spectra in a data
taking (using the same GridPix calibrations) and then mapping the
calibration factor for each calibration spectrum to the gas gain of
each run.

- [X] *INSERT PLOT OF GAS GAIN VS CALIB?*

#+CAPTION: Fit of the gas gain values vs the calculated energy calibration factors
#+CAPTION: for all calibration runs in Run-2. Each run was further sliced into
#+CAPTION: \SI{90}{min} parts for the gas gain determination and fits. *REPLACE BY VERSION WITH FIXED ERRORS AND UNITS + PRETTY*
#+NAME: fig:calib:gasgain_vs_energy_calib
[[~/phd/Figs/gasgain_vs_energy_calibration_factors_-5437145575257130897_2017_placeholder.pdf]]

The behavior between calibration factor and gas gain is mostly linear
as seen in fig *FIGURE INSERT*.

- [ ] *THINK ABOUT INSTEAD OF EXPLAINING HOW THE CALIBRATION IS DONE*
...
Unfortunately, the task is slightly more complicated.

- [ ] *TALK ABOUT DETECTOR BEHAVIOR OVER TIME, REFER TO SUBSECTION*

- [ ] *MENTION IF GAS GAIN VS ENERGY CALIB FITS USE INDIVIDUAL SLICES
  OR MEAN VALUE!*
...
evident from the calibration runs and partially expected due to the
power supply and grounding problems encountered, as well as different
sets of calibrations between Run-2 and Run-3. By binning the data into
short intervals of order one hour, significant fluctuations can be
observed even on such time
...
understood (see sec. [[#sec:calib:causes_variability]]), it allows us to
take action and calibrate the data accordingly.
...
[[~/phd/Figs/median_charge_binned_90min_filtered_crSilver_normalized.pdf]]

- [X] *CHECK NOTES W/ KLAUS AND STATUSANDPROGRESS FOR HOW WE WENT
  THROUGH GAS GAIN SLICING INVESTIGATION*  

- [ ] *REMOVE REFERENCE TO FIXED SPECTRUM AND CALIBRATION RUNS HERE
  AND REWRITE BETTER IN CONTEXT OF PREVIOUS SECTION*
...
- [ ] plot of total charge over time as *main motivation*
...
Reasons for peak position moving:
- electronic causing the effective threshold to change
- change in gas temperature (seems uncorrelated going by
  plots!. *Maybe* correlated to ambient temperature?)
- change in pressure (unlikely, had pressure sensor!)
- change in gas flow (related to above 2) (unlikely, was constant at
  ...
  aware of that!)
- change in grid voltage (real or effective due to charge up)
...
The why is plainly the strong variation, so just explain using 90 min.
