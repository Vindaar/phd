...
studied. The ideal time interval is one in which the resulting energy
distribution is narrow and unimodal approaching a normal
distribution. That is because the background is expected to be
equivalent between all time slices, if some average is considered.
...
parameter that is set to $\SI{25}{min}$ by
default. [fn:configuration_slicing]
...
- [ ] *INSERT FIG OF RIDGELINE FOR MEDIAN ENERGY HISTO FOR DIFFERENT 
  TIMES*
...
- [ ] *REPLACE EVENT FIGURES BY EXAMPLE HISTOGRAM OF DIFFERENT
...
likelihood cut, based on the ideas in \cite{krieger2018search}.

- [ ] *REPHRASE SUCH THAT THIS XRAY DATA BECOMES A REQUIREMENT*
X-ray data taken at multiple energies (possibly taken with an X-ray
tube) are used to define reference distributions for (currently) 3
different geometrical variables:
...
- [X] moved to likelihood method section
