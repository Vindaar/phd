...
Schmick *ADD SCHMICK THESIS TO REFERENCES*
\cite{msc_schmick}. Further, the selection of lines and approach
...
the target material generates Bremsstrahlung with characteristic lines
depending on the target and a continuous portion. A second rotateable
...
larger line width for example in run *FILL MEE KKKKKKKKKKKKK*.
...
- [ ] *LOOK INTO THE CDL TEMPERATURES THAT WE HAVE. CREATE A PLOT OF
...
- [ ] *CREATE PLOT LIKE RIDGE LINE PLOT BUT NOT COMPARING BETWEEN
  ...
  that of course (well I guess that plot _would_ be such a comparison).
...
- explain idea behind it: different targets & filters for different
...
- [ ] *HOW VARIANCE IN TABLE DETERMINED?*
...
- [ ] *UPDATE PEAK POSITIONS IN THE TABLE!*
...
#+CAPTION: line and its energy. Finally, the fitted peak position in the pixel spectrum (easier to gauge
#+CAPTION: the equivalent energy from pixel counting) and variance *HOW DETERMINED???*
#+NAME: tab:cdl:run_overview_tab
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
| Run # | FADC? | Target | Filter | HV / kV | Line                          | Energy / keV | $\langle\mu_{\text{peak}}\rangle$ | $\Delta\mu$ |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   319 | y     | Cu     | Ni     |    15.0 | $\ce{Cu}$ $\text{K}_{\alpha}$ |         8.04 |                            347.77 |        8.49 |
|   320 | n     | Cu     | Ni     |    15.0 |                               |              |                            323.23 |       21.81 |
|   345 | y     | Cu     | Ni     |    15.0 |                               |              |                            271.16 |        6.08 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   315 | y     | Mn     | Cr     |    12.0 | $\ce{Mn}$ $\text{K}_{\alpha}$ |         5.89 |                            223.89 |        8.79 |
|   323 | n     | Mn     | Cr     |    12.0 |                               |              |                            224.78 |        8.92 |
|   347 | y     | Mn     | Cr     |    12.0 |                               |              |                            198.73 |        4.72 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   325 | y     | Ti     | Ti     |     9.0 | $\ce{Ti}$ $\text{K}_{\alpha}$ |         4.51 |                            176.51 |        1.22 |
|   326 | n     | Ti     | Ti     |     9.0 |                               |              |                            173.20 |        2.20 |
|   349 | y     | Ti     | Ti     |     9.0 |                               |              |                            160.86 |        1.25 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   328 | y     | Ag     | Ag     |     6.0 | $\ce{Ag}$ $\text{L}_{\alpha}$ |         2.98 |                            117.23 |        2.02 |
|   329 | n     | Ag     | Ag     |     6.0 |                               |              |                            118.66 |        1.21 |
|   351 | y     | Ag     | Ag     |     6.0 |                               |              |                            106.94 |        2.55 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   332 | y     | Al     | Al     |     4.0 | $\ce{Al}$ $\text{K}_{\alpha}$ |         1.49 |                             55.36 |        1.26 |
|   333 | n     | Al     | Al     |     4.0 |                               |              |                             54.79 |        2.33 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   335 | y     | Cu     | EPIC   |     2.0 | $\ce{Cu}$ $\text{L}_{\alpha}$ |        0.930 |                             32.33 |        2.52 |
|   336 | n     | Cu     | EPIC   |     2.0 |                               |              |                             33.95 |        0.67 |
|   337 | n     | Cu     | EPIC   |     2.0 |                               |              |                             31.51 |        4.76 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   339 | y     | Cu     | EPIC   |     0.9 | $\ce{O }$ $\text{K}_{\alpha}$ |        0.525 |                             25.00 |        0.79 |
|   340 | n     | Cu     | EPIC   |     0.9 |                               |              |                             21.39 |        2.27 |
|-------+-------+--------+--------+---------+-------------------------------+--------------+-----------------------------------+-------------|
|   342 | y     | C      | EPIC   |     0.6 | $\ce{C }$ $\text{K}_{\alpha}$ |        0.277 |                             18.04 |        1.46 |
...
- [ ] *SHOW EXAMPLE FIT WITH RAW+CUTS & FIT PARAMETERS PLOT*

Fig. [[fig:cdl:calibrated_energy_histosc]] shows normalized histograms of all
CDL data after applying basic cuts and calibrating the data in energy
using the charge of the main fitted line and its known energy as a
baseline. Some targets show a wider distribution, due to detector
variability which results in different gas gains and thus different
charges in different runs.
...
- [ ] *AT LEAST THE Cu EPIC 2 KV ROW FIT FUNCTION IS WRONG!! NOT ONLY
  ONE AND NOT Kα!*
...
- [ ] *REWRITE BELOW, FIND CITATION FOR LORENTZ SHAPE OF X-ray transitions*
...
|--------+--------+--------------------------+-----+----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Cu     | Ni     | $\ce{Cu}$ $\text{K}_{α}$ |  15 | $EG^{\mathrm{Cu,esc}}_{\mathrm{K}_{α}}(a,b,N,μ,σ) + EG^{\mathrm{Cu}}_{\mathrm{K}_{α}}(a,b,N,μ,σ)$                                                              |
| Mn     | Cr     | $\ce{Mn}$ $\text{K}_{α}$ |  12 | $EG^{\mathrm{Mn,esc}}_{\mathrm{K}_{α}}(a,b,N,μ,σ) + EG^{\mathrm{Mn}}_{\mathrm{K}_{α}}(a,b,N,μ,σ)$                                                              |
| Ti     | Ti     | $\ce{Ti}$ $\text{K}_{α}$ |   9 | $G^{\mathrm{Ti,esc}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Ti}}_{\mathrm{K}_{β}}(N,μ,σ) + EG^{\mathrm{Ti}}_{K_{α}}(a,b,N,μ,σ) + G^{\mathrm{Ti}}_{K_{β}}(N,μ,σ)$ |
| Ag     | Ag     | $\ce{Ag}$ $\text{L}_{α}$ |   6 | $EG^{\mathrm{Ag}}_{\mathrm{L}_{α}}(a,b,N,μ,σ) + G^{\mathrm{Ag}}_{\mathrm{L}_{β}}(N,μ,σ)$                                                                       |
| Al     | Al     | $\ce{Al}$ $\text{K}_{α}$ |   4 | $EG^{\mathrm{Al}}_{\mathrm{K}_{α}}(a,b,N,μ,σ)$                                                                                                                 |
| Cu     | EPIC   | $\ce{Cu}$ $\text{L}_{α}$ |   2 | $G^{\mathrm{Cu}}_{\mathrm{L}_{α}}(N,μ,σ) + G^{\mathrm{Cu}}_{\mathrm{L}_{β}}(N,μ,σ)$                                                                            |
| Cu     | EPIC   | $\ce{O }$ $\text{K}_{α}$ | 0.9 | $G^{\mathrm{O}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{C}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Fe,esc}}_{L_{α,β}}(N,μ,σ) + G^{\mathrm{Ni}}_{L_{α,β}}(N,μ,σ)$    |
| C      | EPIC   | $\ce{C }$ $\text{K}_{α}$ | 0.6 | $G^{\mathrm{C}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{O}}_{\mathrm{K}_{α}}(N,μ,σ)$                                                                              |
|--------+--------+--------------------------+-----+----------------------------------------------------------------------------------------------------------------------------------------------------------------|


- [ ] *THINK THIS TABLE THROUGH AGAIN. POSSIBLY REPLACE IT BY AN
...
|--------+--------+--------------------------+-----+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Cu     | Ni     | $\ce{Cu}$ $\text{K}_{α}$ |  15 | $G^{\mathrm{Cu,esc}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Cu}}_{\mathrm{K}_{α}}(N,μ,σ)$                                                                                     |
| Mn     | Cr     | $\ce{Mn}$ $\text{K}_{α}$ |  12 | $G^{\mathrm{Mn,esc}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Mn}}_{\mathrm{K}_{α}}(N,μ,σ)$                                                                                     |
| Ti     | Ti     | $\ce{Ti}$ $\text{K}_{α}$ |   9 | $G^{\mathrm{Ti,esc}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Ti}}_{\mathrm{K}_{β}}(N,μ,σ) + G^{\mathrm{Ti}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Ti}}_{\mathrm{K}_{β}}(N,μ,σ)$ |
| Ag     | Ag     | $\ce{Ag}$ $\text{L}_{α}$ |   6 | $G^{\mathrm{Ag}}_{\mathrm{L}_{α}}(N,μ,σ) + G^{\mathrm{Ag}}_{\mathrm{L}_{β}}(N,μ,σ)$                                                                                         |
| Al     | Al     | $\ce{Al}$ $\text{K}_{α}$ |   4 | $G^{\mathrm{Al}}_{\mathrm{K}_{α}}(N,μ,σ)$                                                                                                                                   |
| Cu     | EPIC   | $\ce{Cu}$ $\text{L}_{α}$ |   2 | $G^{\mathrm{Cu}}_{\mathrm{L}_{α}}(N,μ,σ) + G^{\mathrm{Cu}}_{\mathrm{L}_{β}}(N,μ,σ)$                                                                                                                                |
| Cu     | EPIC   | $\ce{O }$ $\text{K}_{α}$ | 0.9 | $G^{\mathrm{O}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{C}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{Fe,esc}}_{L_{α,β}}(N,μ,σ) + G^{\mathrm{Ni}}_{\mathrm{L}_{α,β}}(N,μ,σ)$        |
| C      | EPIC   | $\ce{C }$ $\text{K}_{α}$ | 0.6 | $G^{\mathrm{C}}_{\mathrm{K}_{α}}(N,μ,σ) + G^{\mathrm{O}}_{\mathrm{K}_{α}}(N,μ,σ)$                                                                                           |
|--------+--------+--------------------------+-----+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
- [X] *GENERATE A PLOT OF THE GAS GAINS ENCOUNTERED DURING THE CDL
  ...
  target/filter kind
- [ ] *INSERT RIDGELINE PLOT OF DIFFERENT PROPERTIES PER RUN, SHOWING
