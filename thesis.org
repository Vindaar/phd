...
In the ~reconstruction~ tool each found cluster will then ...
...
#+begin_center
Probably won't end up using this table, instead use the descriptions below.
...
during the first pass of the tool.
...
- igFractionInTransverseRms :: The fraction of all pixels within a
  radius of the transverse RMS around the center.
...
- igLikelihood :: The likelihood value of the cluster for the
  likelihood cut method, explained in detail in section [[#sec:logL:likelihood_of_cluster]].
...
- separate from reconstruction, even though it's also handled by
  ~reconstruction~ due to somewhat different nature of the problems
...
- [ ] How to compute length of events, given
  - shutter mode & length
  - FADC trigger
