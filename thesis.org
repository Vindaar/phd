...
is fitted to the found peak positions of the spectra by charge,
against the known energies of the peaks in the \cefe
spectrum. This yields the calibration factor, $a = m⁻¹$, which can be
used to calibrate all events with _the same_ gas gain. 
...
such gas gains to energies. As we will see in
sec. [[#sec:calib:gasgain_vs_calibration_factor]] the behavior is
linear. The remaining question is the stability of the gas gain over
time, which we will look at next in the context of the general
detector behavior over time.
...
distributions. Further, by applying the energy calibration (as
explained in detail in the next section
[[#sec:calib:energy_calibration_gas_gain]]) based on each different set of
time intervals to the background data, the histograms of the median
cluster energy in the background data was studied. The ideal time
interval is one in which the resulting energy distribution is narrow
and unimodal approaching a normal distribution. That is because the
background is expected to be equivalent between all time slices, if
some average is considered.

- [ ] *INSERT THE PLOT SHOWING THE DIFFERENT DISTRIBUTIONS?*

In this approach a time interval length of $\SI{90}{min}$ was chosen
as a trade off between enough statistics and a stable energy
distribution over time. Of course no run is a perfect multiple of
$\SI{90}{min}$. The last slice smaller than the time interval is
either added to the second to last slice (making it longer than
$\SI{90}{min}$) if it is smaller than some fixed interval or will be
kept as a single shorter interval. This is controlled by an additional
parameter that is set to $\SI{25}{min}$ by default. [fn:configuration_slicing]

- [ ] *INSERT FIG OF MEDIAN ENERGY OF BACKGROUND DATA USING 90 MIN* 
- [ ] *INSERT FIG OF RIDGELINE FOR MEDIAN ENERGY HISTO FOR DIFFERENT TIMES*  

- [ ] *EXPLAIN ~minimumGasGainSlice~*: how we deal with last pieces in
  a run!
  #+begin_src
...
- [ ] *NOTE: The ~optimizeGasGainSliceTime.nim~ tool writes the CSV
...
:PROPERTIES:
:CUSTOM_ID: sec:calib:energy_calibration_gas_gain
:END:

- [ ] *REWRITE ME WITH NEW STRUCTURE IN MIND!*

This is achieved by first performing the
aforementioned gaussian fits to _all_ calibration spectra using the
same GridPix calibrations and then mapping the calibration factor for
each calibration spectrum to the gas gain of each run.

- [X] *INSERT PLOT OF GAS GAIN VS CALIB?*
- [X] *REPHRASE WORD 'UNDERSTANDING' IN SECTION ABOVE*
- [ ] *MOVE EXPLANATION OF THIS FIT TO AFTER DISCUSSION OF GAS GAIN VARIANCE?*  
...
can be seen in fig. [[fig:calib:gasgain_vs_energy_calib]].

- [ ] *HAVE TABLE OF FIT PARAMETERS SOMEWHERE FOR RUN-2 AND RUN-3!*
- [ ] *Instead of single plot as now, have both plots in a subfig for
...
#+begin_center
#+CAPTION: Example event of a \cefe X-ray photon taken with the
