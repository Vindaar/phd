...
from ingrid / reconstruction import readFadcFromH5
...
proc getFadcData(fadcFiles: seq[FadcFile], pedestal: seq[uint16]): Tensor[float] =
  ...
  var fData = ReconstructedFadcRun(
    fadc_data: newTensorUninit[float]([fadcFiles.len, 2560]),
  ...
  for i, file in fadcFiles:
    let data = file.fadcFileToFadcData(pedestal, fadc_ch0_indices).data
    fData.fadc_data[i, _] = data.reshape([1, 2560])
...
proc main(fname: string, runNumber: int) =
  ...
  for run in fileInfo.runs:
    if run == runNumber:
      let fadcFiles = h5f.readFadcFromH5(run)
      let fromData = fadcFiles.getFadcData(pedestal["val", uint16].toSeq1D)
      let fromReal = fadcFiles.getFadcData(pReal["val", uint16].toSeq1D)
...
BE NEEDED, AS THERE IS ENOUGH STUFF ABOUT THIS BEFORE*).
...
- [ ] noise analysis
- [ ] finish trying to deduce the pedestals from real data. Didn't
...
#+begin_src nim :results drawer :tangle code/fadc_testing.nim
import nimhdf5, ggplotnim
import std / [strutils, os, sequtils]
import ingrid / [tos_helpers]
...
#+begin_src sh
./code/fadc_testing -f ~/CastData/data/CalibrationRuns2017_Reco_withFadc.h5 --year 2017
#+end_src
and now for 2018 to generate the CSV for the peak positions of the run
3 data:
#+begin_src sh
./code/fadc_testing -f ~/CastData/data/CalibrationRuns2018_Reco_withFadc.h5 --year 2018
#+end_src
