...
1. the gas flow
2. the chamber pressure via the pressure controller on the inlet *OR
   OUTLET* side
3. the temperature
...
- [ ] *WAS GAS PRESSURE CONTROLLER ON INLET OR OUTLET SIDE??? CHECK
  AND ADJUST ABOVE*
...
- [ ] *CHECK NAME OF BRONKHORST ? OR WHATEVER PRESSURE CONTROLLER AND
  ADJUST IN FOOTNOTE!*
...
losses of the temperature log files, this was simply not prioritized to
implement at the time. The same holds for the gas pressure in the
chamber, which should have been logged using the *CHECK ME NAME*
software of the pressure controller used to control it.
...
- [ ] *INSERT THE PIXEL TEMP GASGAIN PLOT INTO THESIS ROTATED FULL PAGE?*
