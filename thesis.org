...
The maths of the log likelihood expression we use, is mostly straight
forward.
...
If we start from a likelihood ratio of the signal + background
hypothesis over the pure background hypothesis, in the binned case we
can derive the formula used in the Nature paper from first
principles. The number of measured counts in each bin is simply a
...
clusters. For a scan over logL this is precisely given.
...
The InGrid Mk. IV Septemboard detector is a gaseous detector to be
operated at the CAST experiment behind the LLNL telescope (airport
side). 
...
rejection algorithm has been applied for the data during the solar
...
For the numbers of the CAST magnet we therefore get:

- [ ] show equation again
- [ ] show parameters used (L, B, $g_{aγ}$
- [ ] compute the number for the fixed coupling
...
- [ ] explain how the effective area is converted to an efficiency
...
- [ ] potentially rescale number based on area cut off due to edges of
  the chip
- [ ] renormalize from an effective "number" of clusters to a rate
...
- [ ] *TODO*: check the integration of the gaussian weight again. Is
  ...
  the radius, but rather to get the "equivalent area" of the weighted data!
