...
\[
f(μ, σ, N) = \frac{N}{2} · \text{erfc}((x - μ) / (σ * √2))
\]
...
#+CAPTION: \num{25} electrons per DAC value \cite{timepix_manual}. The root of the
#+CAPTION: linear fit corresponds to the position of the noise peak in fig. [[fig:daq:s_curves_example]].
...
- [ ] *ADD THAT AT EVEN LOWER VALUE WE JUST SEE NOISE AND THEN DUE TO
  IMPLEMENTATION DETAILS THE WHOLE THING INVERTS*

- [ ] *ADD ITS OWN SHORT SECTION ABOUT THRESHOLD SCANNING WITH BELOW
  REWRITTEN TEXT*
...
**** Compute electrons per THL DAC value [0/1]                  :noexport:
...
#+begin_src sh
plotCalibration --scurve --chip 3 --runPeriod Run3 --useTeX
#+end_src

The following is essentially the main code required to plot and fit
the SCurves (not the SCurve fit itself):
