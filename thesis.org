...
- [ ] *REWRITE TO USE σ/μ AS OUR 15 PERCENT MATCHES FOR THAT NOT FOR
...
oxygen-free copper with \SI{3}{mm} channels for water to circulate
through the copper body. The body has the same diameter as the
intermediate board and is installed right below. The water circulation
is handled by an off-the-shelf pump and radiator from
...
- [ ] See
  [[file:~/org/Papers/tobias_schiffer_septemboard_cooling_chapter.pdf]]
...
**** TODO Note about variability in GridPix 1
...
inputs required (sec. [[#sec:cdl]]). Next apply it to the data
...
rate. To avoid this issue we perform a linear interpolation for each
cluster with energy $E_α$ between the closest two neighboring X-ray
tube energies $E_β$ and $E_γ$ in each probability density $\mathcal{P}_i$ at the
cluster's properties. With $ΔE = E_β - E_γ$ the difference in energy
between the closest two X-ray tube energies, each probability density
...
distance to each energy is computed. More on this in sec. [[#sec:cdl:cdl_morphing]].
...
Schmick \cite{SchmickMaster}. Further, the selection of lines and approach
follows the ideas used for the single GridPix detector in
\cite{krieger2018search} with some notable differences. For a
reasoning for one particular difference, see appendix
[[#sec:appendix:fit_by_run_justification]].
- [ ] *(RE)WRITE THE FIT BY RUN JUSTIFICATION SECTION AND REFERENCE HERE*
...
order of multiple \si{kV}. The main part of the setup is a rotateable
wheel inside the vacuum chamber, which contains a set of 18 different
positions with 8 different target materials, as seen in
tab. [[tab:cdl:targets]]. The highly energetic electrons interacting with
the target material generates a continuous Bremsstrahlung spectrum
with characteristic lines depending on the target. A second rotateable
...
Fig. \ref{fig:cdl:cdl_setup} shows the whole vacuum test stand, which
...
yellow HDMI cable. In fig. \ref{fig:cdl:detector_setup} we see the
...
sec. [[#sec:cast:data_taking_woes_2017]]), as to even allow to trigger on
$\SI{277}{eV}$ $\ce{C}_{Kα}$ X-rays when running with higher gain
than used at CAST. [fn:shielded_lemo_cable]
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.5\linewidth}
    \centering
    \includegraphics[width=0.95\textwidth]{/home/basti/phd/Figs/CDL/IMG_20190214_155459.jpg}
    \caption{Full setup}
    \label{fig:cdl:cdl_setup}
  \end{subfigure}%
  \begin{subfigure}{0.5\linewidth}
    \centering
    \includegraphics[width=0.95\textwidth]{/home/basti/phd/Figs/CDL/IMG_20190214_191752.jpg}
    \caption{Detector setup}
    \label{fig:cdl:detector_setup}
  \end{subfigure}%
  \label{fig:cdl:cdl}
  \caption{\subref{fig:cdl:cdl_setup} shows the full vacuum test stand containing the X-ray tube with
  the Septemboard detector installed at the rear, visible by the red HV and yellow HDMI cables.
  \subref{fig:cdl:detector_setup} is a view of the detector setup from above. On the left hand side
...
speed somewhat. Table [[tab:cdl:run_overview_tab]] provides an overview of
all data taking runs, the target, filter and HV setting, the main
X-ray fluorescence line targeted and its energy. Finally, the mean
position of the main fluorescence line in the charge spectrum as
determined by the fit and its width is shown. As can be seen the
position moves in some cases significantly between different runs, for
example in case of the $\ce{Cu}-\ce{Ni}$ measurements. Also within a
single run some stronger variation can be seen, evident by the much
larger line width for example in run \num{320} compared to \num{319}.
...
colored by the target/filter combination. It undergoes an extreme,
almost exponential change over the course of the data taking
campaign.
- [ ] *THIS STRONGER VARIATION IS NOT REALLY VISIBLE IN THE WAY WE
...
properties of different gas gains.
- [ ] *HAVE EXPLICIT SUBSECTION IN ABOVE APPENDIX ONLY ABOUT
...
- [ ] *EXTEND THIS TO INCLUDE CITATIONS*
...
**** Reconstruct all CDL data                                   :noexport:
...
**** Generate plots for this section                            :noexport:

- [X] *DISCUSS HOW ~CDL_2019_RECO.h5~ IS GENERATED*

To generate the plots of the above section (and much more) we continue
...
- plots of the target / filter kinds with all fits and the raw / cut
  data as histogram
  - [X] *MISSING THE ANNOTATIONS FOR THE FIT PARAMETERS STILL AND NO
    NLOPT VERSIONS!*
...
**** Generate the ~calibration-cdl-2018.h5~ file
...
**** Get the table of fit lines from code                       :noexport:
...
**** Historical weather data for Geneva during CDL data taking  :noexport:
...
mixture of Gaussian functions is fitted. [fn:pixel_spectra_fitting]
...
expected from the choice of target, filter and chosen voltage and
visible in the data is fit. If no 'argument' is shown in the table it
means each parameter ($N, μ, σ$) is fit. Any specific argument given
implies that parameter is _fixed_ relative to another parameter,
e.g. $μ^{\ce{Ag}}_{Lα}·(\frac{3.151}{2.984})$ fixes the $Lβ$ line of
silver to the fit parameter $μ^{\ce{Ag}}_{Lα}$ of the $Lα$ line with a
multiplicative shift based on the relative eneries of $Lα$ to $Lβ$.
In some cases the amplitude is fixed between different lines where
relative amplitudes cannot be easily predicted, e.g. in one of the
$\ce{Cu}-\text{EPIC}$ the $\ce{C}_{Kα}$ line is fixed to a tenth of
the $\ce{O}_{Kα}$ line. This is done to get a good fit based on trial
and error. Finally, in both $\ce{Cu}-\text{EPIC}$ lines two 'unknown'
Gaussians are added to cover the behavior of the data at higher
charges. It is unclear what the real cause is, in particular in the
lower energy case. For the $\SI{2}{kV}$ data there is a chance it is
related to an aluminum coating that is present on the EPIC filters
*CITE REFERENCE*. It could be a continuous contribution cut by the
absorption edge of aluminum or potentially slight fluorescence. This
explanation doesn't work in the $\SI{0.9}{kV}$ case, which is why the
line is deemed 'unknown'. There is also simply a possibility it is a
case of multi-cluster events, which are too close to be split, but
with properties close enough to a single X-ray as to not be removed by
the cleaning cuts (which gets more likely the lower the energy is).
- [ ] *REWRITE ABOVE PART ABOUT UNKNOWN LINE ACCORDING TO*:
  #+begin_quote
...
cleaning cuts in the purple histogram. The black line is the result of
the fit as described in tab. [[tab:cdl:fit_func_charge]] with the
resulting parameters shown in the box (parameters that were fixed are
not shown). The black straight lines with grey error bands represent
the $3σ$ region around the main fluorescence line, which is used to
extract those clusters likely from the fluorescence line and therefore
known energy. 
...
- [ ] *REWRITE BELOW, FIND CITATION FOR LORENTZ SHAPE OF X-ray
  transitions*
  -> And also this could maybe go to where we first show 
Note that while the physical X-ray transition lines are shaped like a
Lorentzian, the lines as detected by a gaseous detector are entirely
dominated by detector resolution, resulting in Gaussian lines.
...
#+CAPTION: line that is expected and visible in the data is fit. $G$ is a normal Gaussian. If no 'argument'
#+CAPTION: is shown it means each parameter ($N, μ, σ$) is fit. Any specific argument given implies that
#+CAPTION: parameter is _fixed_ relative to another parameter, e.g. $μ^{\ce{Ag}}_{Lα}·(\frac{3.151}{2.984}$ fixes
#+CAPTION: the $Lβ$ line of silver to the fit parameter $μ^{\ce{Ag}}_{Lα}$ of the $Lα$ line with a multiplicative
#+CAPTION: shift based on the relative eneries of $Lα$ to $Lβ$.
#+CAPTION: In some cases the amplitude is fixed between different lines where relative amplitudes cannot be
#+CAPTION: easily predicted, e.g. in one of the $\ce{Cu}-\text{EPIC}$ the $\ce{C}_{Kα}$ line is fixed to a tenth of the
#+CAPTION: $\ce{O}_{Kα}$ line. This is done to get a good fit based on trial and error. Finally, in both $\ce{Cu}-\text{EPIC}$
...
of the main fluorescence line as computed from the fit.
Fig. [[fig:cdl:calibrated_energy_histos]] shows normalized histograms of
all CDL data after applying basic cuts and performing said energy
calibrations. 
...
The desired reference distributions then are simply ...................

The resulting histogram can then interpreted as a probability density
...
5. compute a histogram for each desired cluster property and each
   fluorescence line, defining the reference distribution.

- [ ] mention what kind of binning we use
- [ ] present plot of reference distributions. How?
...
With the main fluorescence lines fitted in all the charge spectra, the
position and line width can be used to compute the energy resolution
of the detector:
...
*** CDL morphing / linear interpolation [/]
:PROPERTIES:
...
**** Plot of temperature in CDL data [/]                        :noexport:
...
* CAST Detector Lab variations and fitting by run                  :noexport:
:PROPERTIES:
:CUSTOM_ID: fig:appendix:fit_by_run_justification
:END:
...
* Morphing of CDL reference spectra                                :Appendix:noexport:
:PROPERTIES:
