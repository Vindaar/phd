...
as a trigger. Table [[tab:cdl:run_overview_tab]] provides an overview of
...
position of that line in the pixel spectrum as determined by the fit
and its variance within the run *HOW DETERMINED?* is shown. The
variability both between runs for the same target and filter as well
as within a run shows the detector was undergoing gas gain changes
similar to the variations at CAST
...
freely opening and closing windows. [fn:weather_at_cdl] 
...
significantly contributing to the detector instability.  
