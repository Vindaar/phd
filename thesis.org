...
*MENTION UV PHOTONS AND HENCE MOLECULAR GASES CALLED QUENCHER GAS*
...
particle, an axion-electron coupling result was computed in *CITE MY MSC*.
...
\times \SI{5}{mm^2}$ region of the detector.
...
*** Calculation of transmission efficiency [0/1] :noexport:
...
non X-ray like clusters. It is clearly visible that the further a
...
- [ ] *MENTION SEPTEMBOARDS ARE NAMED BY LETTERS, WHICH ONE USED IN DETECTOR*  
...
[[file:Figs/occupancy_sparking_septem5chips_300V.pdf]] of run 241 during
...
[[~/phd/Figs/sparking_occupancy_80_quantile_run_268.pdf]]
...
*** Calculation of full detection efficiency
...
thesis, we will now introduce the data acquisition software for the
detector, discuss the data formats used for readout as well as the
logging facilities. Further, we will introduce the calibrations
performed for the Timepix to achieve a correct operation with a
GridPix detector as well as the calibrations for the scintillators and
FADC. Finally, we will present the monitoring tools to monitor the
detector operation (different event displays) which are used for
different data taking purposes.
...
** Timepix Operating Firmware - TOF
:PROPERTIES:
...
- =MCP2210=: A class representing the PT1000 temperature sensors
  installed on the detector via a MCP2210 micro controller. The actual
  micro-controllers with attached PT1000s are MAX31685 models.
...
scintillator paddle in the RD51 *RD51 OR RD52???* laboratory. It is
