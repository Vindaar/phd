...
- [ ] *POSSIBLY SPLIT ZERO SUPPRESSED OUTPUT INTO CHUNKS?*
...
\textbf{H}uman \textbf{I}inteface \textbf{D}evice (HID) driver and
...
The \num{10240} channels of the FADC are part of a cyclic
register. Due to implementation details, the absolute recorded values
of each register is arbitrary. In a pedestal run multiple measurements
($\mathcal{O}(10)$ of a certain length (\SI{100}{ms} in our case)
are performed and the pedestal values averaged. The resulting values
represent a mean value for the typical value in each channel, hence
the name 'pedestal'. To interpret a real measured signal, these
pedestals are subtracted from the recorded
signal. Fig. [[fig:daq:fadc_pedestal_run]] shows the rather peculiar
behavior of the pedestals. The even and odd channels alternate between
values of around \num{2000} and \num{6000}, respectively. The
pedestals drift over time, but the associated time scales are long.

As seen in the config file explanation in section
[[#sec:daq:tos_config_file]], the 

- [ ] *THRESHOLD. 44 mV. TO AVOID NOISE TRIGGERS*
- [ ] *AMPLIFIER SETTINGS*
- [X] *PEDESTAL*

- [ ] *ATTEMPT TO CALCULATE PEDESTAL RUNS FROM FADC DATA FILES BY COMPUTING A TRUNCATED MEAN OF EACH SINGLE CHANNEL. AS THE DATA IS IN
  CHANNEL ORDER, THIS SHOULD BE PRETTY SIMPLE*

#+CAPTION: The FADC pedestal run used for CAST data shows peculiar behavior. Alternatingly
#+CAPTION: the pedestals lie near \num{6000} and near \num{2000}. Within the variance is
#+CAPTION: $\mathcal{O}(30)$ for both. The first 10 channels in total are not shown, as they
#+CAPTION: are outlying by $\sim\num{100}$.
#+NAME: fig:daq:fadc_pedestal_run
#+Attr_LATEX: :width 1\textwidth
[[~/phd/Figs/detector/calibration/fadc_pedestal_split_even_odd.pdf]]
...
- [X] *SIPM CALIBRATED IN BSC OF JANNES*
...
* CAST data taking notes                                          :noexport: :Appendix:
