...
** TODO Use ~booktabs~ everywhere

Simply done by adding ~:booktabs~ to the ~#+ATTR_LATEX:~ above a
table!
...
- [ ] section about the 55Fe source and manipulator, with noexport
...
Using the first X-ray finger run taken in July 2017, the center
...
this. The position almost perfectly ($\sim\SI{0.2}{mm}$) in the center
along one axis and about \SI{1.35}{mm} away from the center along the
other.

- [ ] *REPHRASE ABOVE W.R.T. CORRECT RUN 189 AND WHAT IS WINDOW ETC*
...
#+CAPTION: Cluster center positions of the X-ray finger run 21 from July
#+CAPTION: 2017. The red cross marks the center of all cluster centers,
#+CAPTION: which is the most likely position of the focal spot. It is
#+CAPTION: $\sim\SI{1.35}{mm}$ away from the chip center in one axis and almost
#+CAPTION: about $\sim{0.2}{mm}$ in the other.
#+NAME: fig:cast:xray_finger_centers
[[~/phd/Figs/CAST_Alignment/xray_finger_centers_run_21.pdf]]
...
const path = "/mnt/1TB/CAST/2017/XrayFingerRun/Run_21_170713-11-03/"
# parse data
shell:
  raw_data_manipulation -p ($path) "--runType xray --out /tmp/xray_finger_21.h5"
shell:
  reconstruction "/tmp/xray_finger_21.h5" "--out /t/reco_xray_finger_21.h5"
#+end_src
